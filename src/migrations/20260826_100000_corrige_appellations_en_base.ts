 ...
 */
const MOTIF = "'(Bachelor|Master|Home & Living)'"
...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
    CREATE TABLE IF NOT EXISTS ${sql.raw(TABLE_SAUVEGARDE)} (
  ...
  await db.execute(sql`
    ...
    DECLARE
      ...
      total      INTEGER := 0;
    BEGIN
      FOR colonne IN ${sql.raw(SELECTION_COLONNES)}
      LOOP
        -- Combien de lignes AVANT de rien modifier, pour que le journal
        -- du deploiement dise ce qui allait etre touche, et pas
        -- seulement ce qui l'a ete.
        EXECUTE format('SELECT count(*) FROM %I WHERE %I::text ~ %L',
          colonne.table_name, colonne.column_name, ${sql.raw(MOTIF)})
          INTO a_corriger;
...
export async function down({ db }: MigrateDownArgs): Promise<void> {
  await db.execute(sql`
    ...
    DECLARE
      ...
      total      INTEGER := 0;
    BEGIN
      IF to_regclass(${sql.raw(`'${TABLE_SAUVEGARDE}'`)}) IS NULL THEN
      ...
      FOR cible IN
        ...
        FROM ${sql.raw(TABLE_SAUVEGARDE)} s
          ...
          EXECUTE format(
            'UPDATE %I t SET %I = s.valeur_avant::jsonb
             ...
             WHERE s.nom_table = %L AND s.nom_colonne = %L
               AND t.id::text = s.cle',
            cible.nom_table, cible.nom_colonne, ${sql.raw(`'${TABLE_SAUVEGARDE}'`)},
